...
class Pt extends Ee {
  ...
  async getCategories(e) {
    ...
    if (e.showRecents && r.unshift({ key: "recents", order: -1 }), (a = e.custom) != null && a.length && r.push({ key: "custom", order: 10 }), e.categories) {
      ...
      r = r.filter((l) => n.includes(l.key)), r.sort((l, m) => n.indexOf(l.key) - m.indexOf(l.key));
    } else
...
let K = 0;
function As(s) {
  return M = `picmo-picker-${K++}`, Ts(s);
}
export {
  le as Events,
  Ft as NativeRenderer,
  zt as RecentsProvider,
  It as ViewEvents,
  As as createPicker,
